...
contract ClaimApplication {
    ...
    function submitClaim(uint256 _ClaimAmount) external payable {
        ...
        require(policyContract.isPolicyStarted(msg.sender),
    ...
    function verifyClaim(address _policyHolder)
        external
        hasClaimApplication(_policyHolder)
        onlyInsurer {
        Policy policyContract = Policy(payable(contractRegistry.getContract(REGISTRY_KEY_POLICY)));
    ...
    function rejectClaim(address _policyHolder)
        external
        hasClaimApplication(_policyHolder)
        onlyInsurer {
        Policy policyContract = Policy(payable(contractRegistry.getContract(REGISTRY_KEY_POLICY)));
    ...
    function getClaim(address _policyHolder)
        public
        view
        hasClaimApplication(_policyHolder)
    ...
    function _removeItem(address _policyHolder)
        internal
        hasClaimApplication(_policyHolder) {
        uint256 _claimIndex = indeces[_policyHolder];
