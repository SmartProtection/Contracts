...
contract Policy {
    ...
    modifier hasPolicy(address _policyHolder) {
        require(policyHolders[_policyHolder].policyNumber != 0, "Current user doesn't have a policy");
        _;
    }

    modifier policyShouldBeValid(address _policyHolder) {
        require(isPolicyDeadlineValid(_policyHolder), "Pyment deadline has been missed. The policy is inactive");
     ...
     */
    function makePayment() external payable hasPolicy(msg.sender) policyShouldBeValid(msg.sender) {
        ...
        require(
            _policyPayments.nextDeadline <= _policyDetails.endDate,
    ...
    ) public view hasPolicy(_policyHolder) returns (PolicyDetails memory) {
    ...
    function getPolicyPayments(
        address _policyHolder
    ) public view hasPolicy(_policyHolder) returns (PolicyPayments memory) {
        uint256 _policyNumber = policyHolders[_policyHolder].policyNumber;
        PolicyPayments memory _policyPayments = policyPayments[_policyNumber];
